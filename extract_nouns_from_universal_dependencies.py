#!/usr/bin/env python3
"""
Script to extract nouns from Universal Dependencies v2.17 treebanks.

This script:
1. Downloads the UD v2.17 data (or uses existing data)
2. Reads .conllu files from each language treebank
3. Extracts NOUN lemmas (base forms)
4. Counts their frequencies
5. Saves results to resources/{lang_code}/nouns.txt
"""
...
# Configuration
# Dictionary with language code and whether to lowercase nouns
# German keeps uppercase for nouns, most others use lowercase
LANGUAGES = {
    ...
    'zh_cn': {'name': 'Chinese', 'lowercase': True},
}
...
def parse_conllu_line(line, lang_code='en'):
    ...
    Returns lemma if UPOS is NOUN, else None.
    """
    line = line.strip()
...
def extract_nouns_from_tgz(tgz_path, lang_code):
    ...
    print(f"Processing {lang_code} ({lang_name} - {folder_prefix})...")
...
def save_noun_frequencies(noun_counter, lang_code, output_dir='resources'):
    """
    Save noun frequencies to a text file.
    ...
    # Output file path
    output_file = lang_output_dir / 'nouns.txt'
    ...
    print(f"  Saved {len(sorted_nouns)} unique nouns to: {output_file}")
    print(f"  Total noun occurrences: {sum(noun_counter.values())}")


def main():
    """Main execution function."""
    print("=" * 80)
    print("Universal Dependencies v2.17 Noun Extractor")
    print("=" * 80)
    print()
    ...
    for lang_code in LANGUAGES.keys():
        ...
        if noun_counter:
            save_noun_frequencies(noun_counter, lang_code)
        else:
            print(f"  No nouns extracted for {lang_code}")
